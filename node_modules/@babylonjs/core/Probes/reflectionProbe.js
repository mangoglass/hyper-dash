    ...
    name, size, scene, generateMipMaps, useFloat, linearSpace) {
        ...
        this._renderTargetTexture.onBeforeRenderObservable.add(function (faceIndex) {
            ...
            if (scene.useRightHandedSystem) {
                Matrix.LookAtRHToRef(_this.position, _this._target, Vector3.Up(), _this._viewMatrix);
                if (scene.activeCamera) {
                    _this._projectionMatrix = Matrix.PerspectiveFovRH(Math.PI / 2, 1, useReverseDepthBuffer ? scene.activeCamera.maxZ : scene.activeCamera.minZ, useReverseDepthBuffer ? scene.activeCamera.minZ : scene.activeCamera.maxZ, _this._scene.getEngine().isNDCHalfZRange);
                ...
                if (scene.activeCamera) {
                    _this._projectionMatrix = Matrix.PerspectiveFovLH(Math.PI / 2, 1, useReverseDepthBuffer ? scene.activeCamera.maxZ : scene.activeCamera.minZ, useReverseDepthBuffer ? scene.activeCamera.minZ : scene.activeCamera.maxZ, _this._scene.getEngine().isNDCHalfZRange);
                    scene.setTransformMatrix(_this._viewMatrix, _this._projectionMatrix);
                }
            }
