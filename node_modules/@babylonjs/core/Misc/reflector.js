    ...
    function Reflector(scene, hostname, port) {
        ...
        this._webSocket.onmessage = function (event) {
            ...
            if (StringTools.StartsWith(message, Reflector.SERVER_PREFIX)) {
                var serverMessage = message.substr(Reflector.SERVER_PREFIX.length);
                ...
                _this._handleClientMessage(message);
            }
        };
        ...
        switch (message) {
            case "connected": {
                SceneSerializer.SerializeAsync(this._scene).then(function (serialized) {
                    _this._webSocket.send("load|" + JSON.stringify(serialized));
                });
                break;
            }
        }
    };
    Reflector.prototype._handleClientMessage = function (message) {
        // do nothing
    };
    Reflector.SERVER_PREFIX = "$$";
    return Reflector;
}());
export { Reflector };
